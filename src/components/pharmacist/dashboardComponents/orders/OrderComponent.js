...
const ProductComponent = () => {
  ...
  const [totalOrders,setTotalOrders] = React.useState(0);
   ...
   const getorders = ()=>{
       setTotalOrders(0);
     axios.get("https://ar-medicare-backend.herokuapp.com/api/admin/orders").
     then((res)=> res.data).
     then((r)=>{setOrders(r);
   ...
   const deleteProductByName = (name) => {
     ...
     orders.map(product=>{
       ...
       axios.delete("https://ar-medicare-backend.herokuapp.com/api/orders/"+foundProduct._id).then((response)=>{
    ...
    <Box sx={{paddingLeft:5,
      ...
      <Grid container spacing={3} >
        ...
        <Grid item  sm={12} md={6} lg={4}>
          <TagCard
            title={"Add Product"}
            icon={<AssignmentTurnedInIcon fontSize="large" color="success" onClick={()=>{navigate("/dashboard/orders/addProduct")}}/>}
            category="cash"
            background="#2581c2"
          />
        </Grid>
